...
CREATE TABLE IF NOT EXISTS `infoserv` (
  ...
  KEY `game_fk` (`infogame_fk`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;

--
-- Déchargement des données de la table `infoserv`
--

INSERT INTO `infoserv` (`id`, `serv_name`, `ip`, `password`, `infogame_fk`) VALUES
(1, 'Ark pve', 0, 'osef', 3),
(2, 'ark pvp', 123123, 'osef2', 3),
(3, '7 days pve', 505050, 'not today', 2),
(4, '7 days pvp', 303030, 'good days to die', 2),
(5, 'valheim [MsN]', 545454, 'thor', 4),
(6, 'valheim [MsN] modded', 646464, 'for the good', 4),
(7, '7 days modded', 626262, 'war of walkers', 2);
...
ALTER TABLE `infoserv`
  ADD CONSTRAINT `game_fk` FOREIGN KEY (`infogame_fk`) REFERENCES `game` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
